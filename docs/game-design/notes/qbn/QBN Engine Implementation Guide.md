...
1. **The Narrative Engine** — Decides _which_ storylet to present based on game state
2. **The Text Engine** — Assembles and formats _what_ the player reads
3. **The UI Engine** — Handles _how_ content is presented and how player input is captured
...
### Beyond Uniform Qualities: The Type Taxonomy
...
The Uniform Quality Principle above is how Failbetter originally built Fallen London -- and it works. But Alexis Kennedy himself has come to see its limitations. When all qualities are interchangeable numbers, the engine cannot distinguish between modifying a captain's identity and spending a coin, between a meter that should tick autonomously and a flag that should flip once.

FATE RPG has a parallel problem: every aspect invocation produces an identical +2 bonus. The Fate System Toolkit addressed this with aspect lifespans (Game, Character, Situation, Consequence, Boost), each interacting differently with the fate point economy.

For a production QBN engine, qualities should be **typed**. The [QBN Engine Spec](../../../spec/systems/qbn-engine.md#quality-taxonomy) defines seven types with distinct engine behaviors:

| Type            | Lifespan            | Engine Behavior                                            | Examples                    |
| --------------- | ------------------- | ---------------------------------------------------------- | --------------------------- |
| **Identity**    | Permanent           | High inertia, gates large storylet pools, triggers compels | Captain traits, ship class  |
| **Resource**    | Scarce, tradeable   | Consumed/replenished, economic pressure                    | Fuel, food, currency        |
| **Meter**       | Continuous          | Autonomous change rate, yarnball pressure                  | Hunger, loneliness, boredom |
| **Reputation**  | Slow-moving         | Modified by public actions, gates social access            | Faction standing, notoriety |
| **Consequence** | Multi-session decay | Decays toward baseline, creates recovery arcs              | Ship damage, legal trouble  |
| **Situation**   | Scene-duration      | Cleared on context change, enables prep-and-payoff         | "Scouted route", "docked"   |
| **Momentum**    | One-shot            | Consumed on first use, cannot stockpile                    | Prep bonuses, lucky breaks  |

The implementation impact is significant: the quality state store (shown above as a simple key-value map) should include type metadata, and the engine should enforce type-specific behaviors (e.g., automatically decaying Consequence qualities, clearing Situation qualities on context change, preventing direct modification of Identity qualities except through designated storylets).

This does not invalidate the uniform quality principle for prerequisite checking -- any storylet can still check any quality. But it means the _effect_ system must understand quality types and apply type-appropriate behaviors when modifying them.

### How Quality Types Affect Storylet Priority

> _Added 2026-03-23 from [FATE RPG QBN research](../../../research/2026-03-23_fate-rpg-qbn-lessons/synthesis.md)._

Quality types should influence how the narrative engine prioritizes available storylets, not just which storylets are available. Specifically:

- **Compel storylets** (system-initiated complications tied to Identity or Reputation qualities) should receive a priority boost when the relevant quality is in tension with the current context. A "Notorious Smuggler" at a lawful port should see the compel storylet surface above routine options.
- **Consequence-triggered storylets** (recovery arcs, follow-up complications) should receive elevated priority while the consequence is active -- they represent ongoing situations the player must address.
- **Meter-threshold storylets** (yarnball pressure) should have priority proportional to how far the meter has exceeded its pressure threshold. A crew at Hunger 9 generates more urgent storylets than Hunger 5.
- **Situation-consuming storylets** (prep-and-payoff chains) should surface with moderate priority while the relevant situation quality exists, since they represent time-limited opportunities.

In the selection strategies described in Part II below, this means the priority calculation is not a static number per storylet but a function of the storylet's quality-type interactions with current state.
...
const storylet = {
  ...
  branches: [
    ...
    },
    {
      ...
      prerequisites: [{ quality: 'Trust_CityGuard', op: '>=', value: 10 }],
      effects: [
  ...
  evaluateCondition(condition) {
    ...
    switch (condition.op) {
      ...
      case '<=':
        return currentValue <= targetValue;
      case 'has':
        return currentValue > 0; // quality exists and is positive
      case '!has':
...
function selectHighestPriority(engine) {
  ...
  return available.reduce((best, current) => (current.priority > best.priority ? current : best));
}
```
...
### 2.5 Compel Engine Subsystem

> _Added 2026-03-23 from [FATE RPG QBN research](../../../research/2026-03-23_fate-rpg-qbn-lessons/synthesis.md). See also [QBN Research Lessons](qbn_research_lessons.md)._

Beyond the standard storylet selection strategies above, a production QBN engine needs a **compel engine** -- a subsystem that proactively surfaces system-initiated complication storylets when a player's qualities create dramatic tension with their current context.

The compel engine differs from standard storylet selection in several ways:

1. **Initiation**: Standard storylets are selected by the player from available options. Compel storylets are surfaced by the engine as interrupts or elevated-priority options.
2. **Targeting**: Compels are tied to specific quality types (primarily Identity and Reputation) and activate when those qualities conflict with the current situation context.
3. **Reward structure**: Compels always offer a diegetic reward (in-world resources like fuel, reputation, information) for accepting the complication. This is FATE's bidirectional pressure pattern adapted for QBN -- but without the metagame fate point currency.
4. **Player agency**: The player always has a choice. Refusing a compel costs resources (you spend something to avoid the complication) but is never unfair.

Conceptual implementation:

```javascript
class CompelEngine {
  constructor(narrativeEngine, qualities) {
    this.narrativeEngine = narrativeEngine;
    this.qualities = qualities;
  }

  // Check for applicable compels based on current context
  getActiveCompels() {
    // Get all storylets tagged as compels
    const compelStorylets = this.narrativeEngine
      .getAvailableStorylets()
      .filter((s) => s.tags && s.tags.includes('compel'));

    // Score by tension between quality and context
    return compelStorylets
      .map((storylet) => {
        let tension = 0;
        for (const prereq of storylet.prerequisites || []) {
          const qualityMeta = this.getQualityMeta(prereq.quality);
          if (qualityMeta?.type === 'identity' || qualityMeta?.type === 'reputation') {
            // Higher quality value = stronger compel pressure
            tension += this.qualities[prereq.quality] || 0;
          }
        }
        return { storylet, tension };
      })
      .filter(({ tension }) => tension > 0)
      .sort((a, b) => b.tension - a.tension);
  }
}
```

Key design rule: **every important Identity and Reputation quality must have both beneficial and complicating storylets authored for it**. The compel engine enforces the dual edge that FATE players struggle to self-impose. "Notorious Smuggler" unlocks lucrative black-market jobs AND triggers imperial attention.

In Ravel, the `Compel` annotation on a storylet marks it as system-initiated, affecting how the engine surfaces and prioritizes it. This is being added to the Ravel spec concurrently.

### 2.6 Injection Mechanism (Qualities as Active Agents)

> _Added 2026-03-23 from [FATE RPG QBN research](../../../research/2026-03-23_fate-rpg-qbn-lessons/synthesis.md) — Disco Elysium pattern._

In standard QBN, qualities are passive gates: they determine which storylets are available, but they do not generate content. The injection mechanism extends this so that strong qualities can **insert themselves** into storylets they do not gate, volunteering commentary or alternative options.

Example: A captain with Paranoia 8 enters a routine trade negotiation storylet. The storylet was not authored with Paranoia in mind, but the injection mechanism detects that Paranoia exceeds its assertiveness threshold (say, 6) and inserts an additional branch: _"Something about this deal feels wrong. The merchant's smile is too wide, too practiced. [Paranoid interpretation]"_

The mechanism requires:

1. **Assertiveness thresholds**: Each quality type has a threshold above which it becomes "assertive" -- eager to inject into unrelated storylets.
2. **Injection points**: Storylets declare slots where injections can occur (e.g., before branch selection, during outcome narration, as additional branches).
3. **Injection content**: Quality-tagged content blocks that can be inserted into compatible injection points. These are authored alongside the quality definition, not alongside every individual storylet.
4. **Relevance filtering**: Not every assertive quality should inject into every storylet. Tag-based relevance matching (e.g., a "social" quality injects into "negotiation" storylets but not "combat" storylets) limits noise.

```javascript
class InjectionEngine {
  constructor(qualities, injectionRegistry) {
    this.qualities = qualities;
    this.registry = injectionRegistry; // quality -> injection content blocks
  }

  // Get injections applicable to a storylet
  getInjectionsFor(storylet) {
    const injections = [];
    for (const [qualityName, blocks] of this.registry.entries()) {
      const value = this.qualities[qualityName] || 0;
      const meta = this.getQualityMeta(qualityName);
      if (value >= (meta?.assertivenessThreshold || Infinity)) {
        for (const block of blocks) {
          // Check tag compatibility
          if (this.tagsOverlap(block.relevantTags, storylet.tags)) {
            injections.push({
              quality: qualityName,
              value,
              content: block.content,
              type: block.type, // "branch", "commentary", "alternative"
            });
          }
        }
      }
...
**Caution**: This is the least proven pattern from the research. Disco Elysium is a single-player game with 24 predefined skills and heavily authored injection content. Whether injection scales to a multiplayer engine with hundreds of qualities -- and what the authoring cost looks like -- remains an open question. The Ravel language is being extended with an `Interjection` mechanism to support this pattern, but it should be considered experimental.

### 2.7 Effect Application
    ...
    for (const effect of effects) {
      ...
      switch (effect.op) {
        ...
        case 'random':
          ...
          newValue = effect.min + Math.floor(Math.random() * (effect.max - effect.min + 1));
          break;
        case 'clamp':
...
class ConditionalTextEngine {
  ...
  processConditionals(text) {
    ...
    return text.replace(pattern, (match, quality, op, value, content) => {
      ...
      switch (op) {
        case '==':
          condition = qualityValue == targetValue;
          break;
        case '!=':
          condition = qualityValue != targetValue;
          break;
...
class PronounEngine {
  ...
  expandPronouns(text, genderQuality = 'Gender') {
    ...
    for (const [placeholder, pronoun] of Object.entries(pronouns)) {
      ...
      result = result.replace(new RegExp(`\\[${placeholder}\\]`, 'gi'), (match) =>
...
class WebUIEngine extends UIEngine {
  ...
  async presentOutcome(outcome, qualityChanges, textEngine, describer) {
    ...
    return new Promise((resolve) => {
      this.container.querySelector('.continue-button').addEventListener('click', resolve);
    });
  }
...
class ConsoleUIEngine extends UIEngine {
  ...
  async presentOutcome(outcome, qualityChanges, textEngine, describer) {
    ...
    if (qualityChanges.length > 0) {
      console.log('\n--- Effects ---');
      for (const change of qualityChanges) {
        console.log(describer.describeChange(change.quality, change.oldValue, change.newValue));
      }
    }
...
class QBNGame {
  ...
  async run() {
    while (true) {
      ...
      const available = this.narrativeEngine
        .getAvailableStorylets()
      ...
      const branchId = await this.ui.presentStorylet(storylet, availableBranches, this.textEngine);
      ...
      // 6. Present outcome
      await this.ui.presentOutcome(branch.outcome, changes, this.textEngine, this.qualityDescriber);
...
class SaveSystem {
  ...
  save(slot = 'default') {
    const saveData = {
      ...
      playHistory: Array.from(this.game.repeatabilityManager.playHistory.entries()),
    };
...
const branchingStorylets = [
  ...
  // Bottleneck: both paths lead here
  {
    ...
    content:
      '[if Alliance == Rebels]Your rebel allies stand beside you.[/if][if Alliance == Empire]Imperial troops form ranks behind you.[/if] The enemy approaches...',
  },
];
```
...
const cyclicalStorylets = [
  // Monthly event that recurs
  {
    ...
    repeatable: true,
    content:
      "[if Year == 1]The market is bustling—you're still learning the vendors' faces.[/if][if Year >= 2]The familiar market stalls welcome you back.[/if]",
    branches: [
      {
        id: 'trade',
        text: 'Trade goods',
...
class StoryletDebugger {
  ...
  validateDatabase() {
    ...
    for (const storylet of this.database) {
      ...
      const prereqQualities = new Set((storylet.prerequisites || []).map((p) => p.quality));

      // Check if any storylet sets the required qualities
      const canBeReached = this.database.some((other) =>
        other.branches?.some((branch) =>
          branch.effects?.some((effect) => prereqQualities.has(effect.quality))
        )
      );
  ...
  getUnderplayedStorylets(allStorylets, threshold = 0.1) {
    ...
    for (const event of this.events) {
      if (event.type === 'storylet_viewed') {
        viewCounts.set(event.storyletId, (viewCounts.get(event.storyletId) || 0) + 1);
      }
    }
...
The QBN approach trades the precision of branching narrative for the flexibility of emergent story. When well-designed, the result feels both authored and personal—players discover _their_ story within the possibility space you've created.
...
- Failbetter Games' design diaries on _Fallen London_ and StoryNexus
...
_This guide synthesizes research from the interactive fiction community, particularly the pioneering work of Failbetter Games and the analytical contributions of Emily Short, Alexis Kennedy, Bruno Dias, and others who have shaped our understanding of quality-based narrative systems._
