...
describe('generateTests', () => {
  ...
  it('preserves custom import lines from existing source', () => {
    ...
    const boundImpl = `it("User registers.", async () => {\n  const { csrfToken } = await getAuthForm("/auth/sign-up");\n  expect(csrfToken).toBeTruthy();\n});`;
    const existingSource = [
      '// Code generated by acceptance-pipeline.',
      '// Source: specs/US03.txt',
      '',
      'import { SELF } from "cloudflare:test";',
      'import { describe, it, expect } from "vitest";',
      'import { getAuthForm } from "./helpers";',
      '',
      'describe("US03", () => {',
      '',
      boundImpl,
    ...
    const output = generateTests(feature, existingSource);
    expect(output).toContain('import { getAuthForm } from "./helpers";');
  });
});
