...
1. **Design constraints on fresh code** — findings that tell the implementer *how to write something correctly from the start* (e.g., "use .bind()", "add optimistic lock", "validate Content-Type"). These have no independent existence: the code doesn't exist yet, so there's nothing to fix. Implementing them as separate tasks forces a build-wrong-then-fix cycle.

2. **Fixes to pre-existing code** — findings that modify files that already existed *before* this feature branch (e.g., `requireAuth.ts`, `worker.ts`). These are genuinely independent: the code exists, the fix is bounded, ralph can process them at any time.
