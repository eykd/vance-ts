...
```typescript
export async function handleInviteMember(c: Context): Promise<Response> {
  const session = c.get('session');
  const orgId = c.req.param('orgId');
  const { email, role } = await c.req.json<{ email: string; role: OrgRole }>();

  try {
    const invitation = await inviteService.invite(c.env.DB, session.userId, orgId, email, role);

    // Send invitation email
    await sendInvitationEmail(c.env, email, invitation.token);

    return c.json({ success: true, invitationId: invitation.id });
  } catch (error) {
...
export async function handleRemoveMember(c: Context): Promise<Response> {
  ...
  try {
    await membershipService.removeMember(c.env.DB, session.userId, orgId, targetUserId);
...
```typescript
export async function handleTransferOwnership(c: Context): Promise<Response> {
  const session = c.get('session');
  const orgId = c.req.param('orgId');
  const { newOwnerId } = await c.req.json<{ newOwnerId: string }>();

  try {
    await membershipService.transferOwnership(c.env.DB, session.userId, orgId, newOwnerId);
