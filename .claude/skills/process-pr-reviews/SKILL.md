...
import json, sys
sentinel = "<!-- reviewer:"
# Check both the correct banner and the \!-- variant produced by zsh history expansion
processed_variants = ["<!-- processed -->", "<\\!-- processed -->"]
comments = json.load(open('/tmp/pr-comments.json'))
unprocessed = [
    c for c in comments
    if sentinel in c['body'] and not any(p in c['body'] for p in processed_variants)
]
...
**Important**: Do NOT use `printf '<!-- processed -->'` even with single quotes — in an interactive zsh session the `!` can still be escaped to `\!`, producing a malformed `<\!-- processed -->` banner that the idempotency check won't recognise. Use a Python script written to a temp file instead:
...
cat > /tmp/mark_processed.py << 'PYEOF'
import json, sys, subprocess

comment_id = sys.argv[1]
task_list = sys.argv[2]
today = sys.argv[3]
owner_repo = sys.argv[4]

comments = json.load(open('/tmp/unprocessed.json'))
comment = next(c for c in comments if str(c['id']) == comment_id)
original_body = comment['body']

banner = f"<!-- processed -->\n> \u2705 **Processed** \u2014 Beads tasks: {task_list}\n> Run on: {today}\n\n---\n\n"
new_body = banner + original_body

payload = json.dumps({"body": new_body})
open('/tmp/patch_body.json', 'w').write(payload)
PYEOF
python3 /tmp/mark_processed.py "${COMMENT_ID}" "${TASK_LIST}" "${TODAY}" "${OWNER_REPO}"

gh api --method PATCH "/repos/${OWNER_REPO}/issues/comments/${COMMENT_ID}" \
  --input /tmp/patch_body.json > /dev/null && echo "Marked comment ${COMMENT_ID} processed"
```
