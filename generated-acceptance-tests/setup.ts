...
/**
 * Database migrations, inlined to avoid Node.js file-system access from the Workers runtime.
 *
 * SOURCE OF TRUTH: migrations/0001_better_auth_schema.sql through migrations/0008_audit_event.sql
 *
 * These constants duplicate the SQL from migration files and MUST be kept in sync manually.
...
// Apply all migrations once per worker isolate (i.e., once per test file).
beforeAll(async () => {
  await applyD1Migrations(env.DB, ALL_MIGRATIONS);
});
