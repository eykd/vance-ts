...
- **FR-002**: Pipeline MUST generate approximately 12,000 star systems at unique integer coordinates using the spiral galaxy algorithm specified in the game design spec §3.1, including: size-to-radians conversion (`sx = 2.0 * size[0] * π / 360.0`), cloud size parameters (`round(deg / π * sx / 1.7) * dynSizeFactor`), quadratic center-biased radial distribution (`(randint(0,10000)/10000)²`), sin-for-x/cos-for-y coordinate mapping in the elliptic starfield, and 1–5 degree random angular steps along spiral arms.
- **FR-002a**: The galaxy generator MUST implement a three-level generator hierarchy: `generateSpiralGalaxyCoords` (galaxy level — size conversion, arm iteration, limit tracking) → `generateSpiralArmCoords` (arm level — spiral walk, cloud placement, division-by-zero guards) → `generateEllipticStarfieldCoords` (cloud level — center-biased elliptical star placement). Each level MUST be implemented as a generator function (`function*` / `yield`) and accept camelCase parameters matching the algorithm specification. A single shared PRNG instance MUST be passed through all three levels with state advancing sequentially.
