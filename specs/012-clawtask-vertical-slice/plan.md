...
(env.DB.prepare(
  ...
  // NOT:
  env.DB.prepare(
    `INSERT INTO workspace (id, user_id, ...) VALUES ('${workspaceId}', '${userId}', ...)`
  ));
```
...
| `POST /app/_/actions/:id/activate` | _(none)_ | Row swap is sufficient |
| `POST /app/_/actions/:id/complete` | _(none)_ | Row swap is sufficient |
...
html`<button
  aria-label="Activate: ${action.title}"
  hx-post="/app/_/actions/${action.id}/activate"
  ...
>
  Activate
</button>`;
...
html`<button
  ...
  hx-post="/app/_/actions/${action.id}/complete"
  ...
>
  Complete
</button>`;
